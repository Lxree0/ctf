...
def login():
    ...
    if request.method == "POST":
        ...
        # -------------------------------------------------------------------
        # ADMIN LOGIN (NO PASSWORD, SOLO is_admin)
        # -------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------
        # USER LOGIN (NO PASSWORD + 3 ORE DI VALIDITÀ)
        # -------------------------------------------------------------------
        cursor.execute("SELECT * FROM users WHERE username=%s", (username,))
        ...
        if not user:
            ...
            # Controllo scadenza 3 ore 
            if not user.get("is_admin") and user["created_at"] is not None:
                last_login = user["created_at"]

                if now - last_login > timedelta(hours=3):
                    # elimina tutti i progressi dell'utente
                    cursor.execute("DELETE FROM submissions WHERE user_id=%s", (user["id"],))

                    
                    cursor.execute("DELETE FROM challenge_completion WHERE user_id=%s", (user["id"],))
